# Northwind Traders Management report

## Overview
This PowerBI report project is developed for Northwind Traders, a global import and export company specializing in gourmet food products. The purpose of this dashboard is to provide the executive team with insights into the company's performance across the following commercial key areas: sales trends, product performance, key customers, and shipping costs; As well as global sales person performance and their sales by country and product category. 
...
## Data set:
 ...
 There are 7 csv files provided by the company and 2 tables created by myself. One is a date table, intended to free data model's memory by disabling the auto-date/time feature as well as to work smoothly with time series and the other is a blank table whose role is to centralise all the measures created.
  ...
  For detailed information about the meaning of each column please consult the data_dictionary.csv file.


### Report structure and abbreviations.
...
Considering the requirement from above, I have created and confirmed with business team the following questions which would be the compass for this project:
...
- Top 10 customers by: Net revenue, discounts, shipping cost by year or for the entire period.
- Please provide the net revenue, discounts, shipping cost by product category and product name by year or for the entire period.
- Please provide quarterly net revenue evolution over the entire period.
- Please provide the shipping cost for each supply chain partner by year or for the entire period.

**Team Performance dashboard questions:**
- Please provide a sales representative summary visual with the following information:
    - One big card called "Employee profile" containing the following:
        ...
        - 2 charts reflecting the gross sales generated by each sales rep., one per product category and the other per country for each year or all years acummulated.
    - One chart with total discounts offered by each employee for each year and all years acummulated.
    - One chart showing worldwide ranking by gross sales for all the employees, the product they sold most and total sales generated by that specific product.
